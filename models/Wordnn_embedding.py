...
class WordnnEmbedding(nn.Module):
    ...
    def __init__(
        ...
        hidden_size=768,
        embedding_dim=64,
        bros_embedding_path="/bros-base-uncased/word_embeddings.pt",
    ...
    def forward(self, img, batched_inputs: dict[str, torch.Tensor], stride=1):
        """Forward computation
        Args:
        ...
        for iter_b in range(batch_b):
            ...
            if short_length_w > 0:
                for word_idx in range(short_length_w):
                    ...
                    w_start, h_start, w_end, h_end = bbox.round().int().tolist()

                    if self.use_UNK_text:
                        chargrid_map[
                            iter_b, h_start : h_start + h_end, w_start : w_start + w_end
                        ] = 100
                    else:
                        chargrid_map[
                            iter_b, h_start : h_start + h_end, w_start : w_start + w_end
        ...
        chargrid_map = chargrid_map.half()
