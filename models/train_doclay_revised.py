...
def create_collate_fn(processor, class_weights):
   ...
   def collate_fn(batch):
       ...
       # This check is now a safeguard; with pre-filtered data, it's less likely to be triggered.
       if not valid_indices:
           # In the rare case a batch is still invalid, returning an empty dict is safer than None.
           return {}
...
if __name__ == "__main__":
   ...
   print("Filtering out invalid samples from datasets...")
   train_dataset = train_dataset.filter(lambda example: example.get("class_strs") and len(example["class_strs"]) > 0)
   val_dataset = val_dataset.filter(lambda example: example.get("class_strs") and len(example["class_strs"]) > 0)
   print(f"Filtered train dataset size: {len(train_dataset)}, Filtered val dataset size: {len(val_dataset)}")
   # --- END OF FIX ---
