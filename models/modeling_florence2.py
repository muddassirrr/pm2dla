...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    def __init__(self, config: Florence2Config):
        ...
        self.wordgrid_embeddings = WordnnEmbedding(
            bros_embedding_path="/home/muddassir/Desktop/LayoutAnalysis/code_modularized/models/word_embeddings.pt"
        )
        vision_config = config.vision_config
        vision_config.in_chans = 64
        self.grid_tower = DaViT.from_config(vision_config)
    ...
    def _encode_image(self, pixel_values, grid_data):
        if len(pixel_values.shape) == 4:
            batch_size, C, H, W = pixel_values.shape
            if len(grid_data) != batch_size:
                raise ValueError(
                    f"batch size mismatch: {len(grid_data)} words, {batch_size} images"
                )
            T = 1
            char_grid = self.wordgrid_embeddings(pixel_values, grid_data)
            grid_x = self.grid_tower.forward_features_unpool(char_grid)
            x = self.vision_tower.forward_features_unpool(pixel_values)
            x += grid_x
        else:
            raise ValueError(f"invalid image shape {pixel_values.shape}")
    ...
    def forward(
        ...
        grid_data: Dict[str, torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        decoder_input_ids: Optional[torch.LongTensor] = None,
        decoder_attention_mask: Optional[torch.LongTensor] = None,
        head_mask: Optional[torch.Tensor] = None,
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_features = self._encode_image(pixel_values, grid_data)
    ...
    def generate(self, input_ids, inputs_embeds=None, pixel_values=None,grid_data=None, **kwargs):
        if inputs_embeds is None:
            # 1. Extra the input embeddings
            if input_ids is not None:
                inputs_embeds = self.get_input_embeddings()(input_ids)
            # 2. Merge text and images
            if pixel_values is not None:
                image_features = self._encode_image(pixel_values, grid_data=grid_data)
                inputs_embeds, attention_mask = (
                    self._merge_input_ids_with_image_features(
                        image_features, inputs_embeds
                    )
