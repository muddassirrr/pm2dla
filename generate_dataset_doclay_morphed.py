...
class COCODataset(torch_dataset):
    ...
    def filter_img_ids (self, img_ids):
        ids_to_remove = set()
        print("Filtering image and pickle file existence...")
        for i, img_id in enumerate(img_ids):
            # Add a progress indicator for large datasets
            if (i + 1) % 5000 == 0:
                print(f"  ...scanned {i + 1} / {len(img_ids)} entries.")
            ...
            if not img_path.exists():
                ...
                continue  # Skip to next id if image is missing

            # 2. Check if the corresponding pickle file exists
            pickle_file_name = img_info["file_name"][0:-4] + ".pdf.pkl"
            pickle_path = self.word_pickles_dir / pickle_file_name
            if not pickle_path.exists():
                ids_to_remove.add(img_id)
        
        print(f"Found {len(ids_to_remove)} entries with missing files. They will be removed.")
